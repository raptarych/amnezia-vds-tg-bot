"""Tests for the bot logging configuration."""
...
def test_configure_logging_writes_to_file(tmp_path: Path) -> None:
    ...
    logger = logging.getLogger("amnezia.bot.test")
    logger.info("hello from bot logging test")

    for handler in logging.getLogger().handlers:
        handler.flush()

    content = log_file.read_text(encoding="utf-8")
    assert "hello from bot logging test" in content
